...
public class MosasaurusCameraScript : MonoBehaviour {
	public GameObject target;
	public float turnSpeed=.2f;
	public GameObject mosasaurusCamera;
	...
	void Update(){
		...
		if (Input.GetAxis ("Mouse ScrollWheel") > .001f) {
			cameraDistance=cameraDistance+Time.deltaTime*30f;
		}else if(Input.GetAxis ("Mouse ScrollWheel") <-.001f){
			cameraDistance=cameraDistance-Time.deltaTime*30f;
		}
		mosasaurusCamera.transform.localPosition = new Vector3 (0f,cameraDistance,-2f*cameraDistance);
	}
